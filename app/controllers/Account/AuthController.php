...
class AuthController extends AccountController {
    ...
    public function postLogin($target='backend')
    {
        ...
        if (Auth::user()->attempt(array('email' => $email, 'password' => $password),true))
        {
    ...
    public function postAccountLogin($target='account'){
        $input = \Input::all();
        ...
        if(\Request::ajax()){
            ...
            try{
                ...
                \Auth::account()->attempt(array('email' => $email, 'password' => $password),true);
                //if(Auth::account()->check()){
                print_r($input);
                //}
        ...
        try{
            ...
            if (\Auth::account()->attempt(array('email' => $email, 'password' => $password),true)){
                ...
                \Session::put("error_message","Invalid Username and Password Combination");
                return \Redirect::back();
            }
        }catch(\Exception $e){
            \Session::put("error_message",$e->getMessage());
